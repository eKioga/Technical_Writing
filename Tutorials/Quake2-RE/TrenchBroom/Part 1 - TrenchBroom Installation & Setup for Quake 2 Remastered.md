...
4. Extract the contents of the downloaded .7z file to any folder on your system.
	...
	1. Create a folder on your Desktop or in your Documents folder and name it "TrenchBroom".
	2. Open the .7z file, select every file within, and drag them to the TrenchBroom folder you created. That's it! Your installation is now complete!
		**Note**: *If you're unable to open the 7z file, it's possible you forgot to download 7zip or WinRAR (seen in the Prerequisite section above).* If you're not sure which, grab 7zip for simplicity sake.
...
13. Copy the Quake 2 icon file over to the Quake2RE folder by performing the following steps:
	1. Navigate to "TrenchBroom\\games\\Quake2"
	2. Copy "Icon.png"
	3. Navigate to "TrenchBroom\\games\\Quake2RE"
	4. Paste "Icon.png"
...
7. Within the "Game Engines" window, perform the following steps:
	...
	2. In the "Name" text box, input "Quake 2-RE".
...
8. At the "Preferences" window, perform the following steps:
	...
	2. Click **OK** to leave the "Preferences" window.

9. At the "Select Game" window perform the following steps:
	1. Ensure that the Quake 2-RE entry is selected.
...
6. Within the Compile window, perform the following steps:
	...
	2. Input the following "Name" and "Working Directory" text for the new profile.
...
9. Populate the "Tool Path" and "Parameter" text boxes for each "Run Tool" entry by performing the following steps:
	**Note**: *To input the correct "Tool Path" for your system, you will need to know where you extracted ericw-tools* back in Step 4.
	1. Run Tool #1
		- **Tool Path**: `ericw-tools-win64/bin/qbsp.exe`
		- **Parameters**: `-nostat -nopercent -q2bsp ${MAP_BASE_NAME}`
	2. Run Tool #2
		- **Tool Path**: `ericw-tools-win64/bin/vis.exe`
